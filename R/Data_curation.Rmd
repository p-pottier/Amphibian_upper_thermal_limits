...
n_distinct(d.raw$ref) # 214 studies
...
# 4.95 (+/- 10.2) species per study on average (range: 1-75)
# 5.91 (+/- 10.7) populations per study on average (range: 1-75)
# 14.46 estimates (+/- 27.2) estimates per study on average (range: 1-230)
...
n_distinct(d.curated$es_ID) # 2890 effect sizes
n_distinct(d.curated$ref) # 199 studies
n_distinct(d.curated$species) # 588 species
n_distinct(d.curated$population_ID) # 1111 populations 
n_distinct(d.curated$cohort_ID) # 2682 cohorts
n_distinct(d.curated$family)

test<-d.curated %>% group_by(ref) %>% 
          summarise(n_sp = n_distinct(species),
                    n_pop = n_distinct(population_ID),
                    n_es = n_distinct(es_ID)) %>% 
          ungroup() %>% 
          summarise(`Number of species per study` = mean(n_sp),
                    ...
                    `Minimum number of estimates per study` = min(n_es),
                    `Maximum number of estimates per study` = max(n_es))
# 5.06 (+/- 10.3) species per study on average (range: 1-74)
# 6.07 (+/- 10.8) populations per study on average (range: 1-74)
# 14.52 estimates (+/- 27.9) estimates per study on average (range: 1-230)
...
d.curated %>% group_by(species) %>% 
          ...
          summarise(`Number of populations per species` = mean(n_pop),
                    ...
                    `Minimum number of estimates per species` = min(n_es),
                    `Maximum number of estimates per species` = max(n_es))
# 1.93 (+/- 2.36) populations per species on average (range: 1-26)
# 4.91 (+/- 12.1) estimates per species on average (range : 1-146)
